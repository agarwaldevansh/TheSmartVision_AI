...
// const BASE_URL = "https://d849e508-dc56-4ee1-9847-bae5786d872a-00-37cx0isfo7346.sisko.replit.dev";
...
//             formData.append('file', { uri: sourceUri, name: 'text.jpg', type: 'image/jpeg' });
...
//             } else {
//                 setAiResult(data.extracted_text || "No text found");
//             }
...
//             <Text style={styles.headerTitle}>TEXT <Text style={styles.cyanText}>RECOGNITION</Text></Text>
...
//                         <TouchableOpacity
//                             style={[styles.tool, isErasing && styles.toolActive]}
//                             onPress={() => { if (isErasing) canvasRef.current?.draw(); else canvasRef.current?.erase(); setIsErasing(!isErasing) }}
//                         >
//                             <Text style={styles.icon}>{isErasing ? '✍️' : '🧽'}</Text>
//                         </TouchableOpacity>

//                         <TouchableOpacity style={styles.tool} onPress={() => canvasRef.current?.undo()}>
//                             <Text style={styles.icon}>↩️</Text>
//                         </TouchableOpacity>

//                         <TouchableOpacity style={styles.toolRed} onPress={() => { setCanvasMemory(null); canvasRef.current?.clearSignature() }}>
//                             <Text style={styles.icon}>🗑️</Text>
//                         </TouchableOpacity>
...
//                             {[2, 8, 16].map(s => (
//                                 <TouchableOpacity key={s} style={[styles.sizeDot, penSize === s && styles.activeSize]} onPress={() => setPenSize(s)}>
//                                     <View style={[styles.dot, { width: s + 2, height: s + 2 }]} />
//                                 </TouchableOpacity>
//                             ))}
//                         </View>
//                     </View>
...
//                             penColor="#000000"
//                             webStyle={`
//                             .m-signature-pad { background-color: #ffffff; border: none; box-shadow: none; }
...
//                 <LinearGradient colors={['#6366f1', '#a855f7']} style={styles.mainBtn}>
//                     {isAnalyzing ? <ActivityIndicator color="#fff" /> : <Text style={styles.mainBtnTxt}>ANALYZE DATA</Text>}
//                 </LinearGradient>
//             </TouchableOpacity>
...
//                                 <Text style={styles.resultLabel}>Detected Content:</Text>
//                                 <View style={styles.resultBox}>
//                                     <Text style={styles.resultContentText}>{aiResult}</Text>
//                                 </View>
//                             </ScrollView>
//                             <View style={styles.modalFooter}>
//                                 <TouchableOpacity style={styles.copyBtn} onPress={async () => { await Clipboard.setStringAsync(aiResult); Alert.alert("Success", "Copied!"); }}>
...
export default function TextScreen() {
    ...
    const handleOK = async (signature) => {
        try {
            // 1. Guard check: Ensure we actually have drawing data
            if (!signature) {
                Alert.alert("Empty", "Please draw something first.");
                setIsAnalyzing(false);
                return;
            }

            // 2. Fool-proof Base64 extraction
            const base64Data = signature.includes(',') ? signature.split(',')[1] : signature;

            // 3. Create a unique filename
            const filename = `${FileSystem.cacheDirectory}sketch_${Date.now()}.jpg`;

            // 4. Save the pure image file to the device (FIXED ENCODING STRING)
            await FileSystem.writeAsStringAsync(filename, base64Data, {
                encoding: 'base64'
            });

            // 5. Send the perfect file to Replit
            triggerAI(filename);

        } catch (err) {
            console.error("Canvas Save Error:", err);
            Alert.alert("Error", "Failed to process sketch layout.");
            setIsAnalyzing(false);
        }
    };
    const triggerAI = async (sourceUri) => {
        ...
        try {
            ...
            setConfidence(data.confidence ? data.confidence.replace('%', '') : "0");
            setModalVisible(true);
    ...
    return (
        <View style={styles.container}>
            ...
            {inputMode === 'upload' ? (
                ...
                <View style={styles.drawArea}>
                    <View style={styles.toolbarGlass}>
                        ...
                        <View style={styles.vDivider} /><View style={styles.sizeGroup}>{[2, 8, 16].map(s => (
                            <TouchableOpacity key={s} style={[styles.sizeDot, penSize === s && styles.activeSize]} onPress={() => setPenSize(s)}>
                                <View style={[styles.dot, { width: s + 2, height: s + 2 }]} />
                            </TouchableOpacity>
                        ))}</View>
            ...
            <TouchableOpacity
                ...
                onPress={() => {
                    if (inputMode === 'upload') {
                        if (!imageUri) return Alert.alert("Empty", "Upload a photo");
                        setIsAnalyzing(true);
                        triggerAI(imageUri);
                    } else {
                        setIsAnalyzing(true);
