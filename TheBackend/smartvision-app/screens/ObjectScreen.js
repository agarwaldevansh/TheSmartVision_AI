...
export default function ObjectScreen() {
    ...
    const triggerAI = async (sourceUri) => {
        ...
        try {
            ...
            formData.append('file', { uri: sourceUri, name: 'object.jpg', type: 'image/jpeg' });

            const response = await fetch(`${BASE_URL}/predict-doodle`, {
                method: 'POST',
                body: formData,
            ...
            if (data.doodle) {
                setAiResult(`Object: ${data.doodle.toUpperCase()}`);
            } else {
                setAiResult(data.error || "No object found");
            }
    ...
    return (
        <View style={styles.container}>
            <Text style={styles.headerTitle}>OBJECT <Text style={styles.cyanText}>VISION</Text></Text>
